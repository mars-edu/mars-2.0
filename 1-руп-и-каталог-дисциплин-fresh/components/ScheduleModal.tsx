...
import { DayOfWeek, ScheduleFormData, DayTime, IndividualJournal } from '../types';
...
// Mock Administrator Configured Time Slots
const TIME_SLOTS = [
  { id: 1, label: '1 пара', start: '08:30', end: '09:50' },
  { id: 2, label: '2 пара', start: '10:00', end: '11:20' },
  { id: 3, label: '3 пара', start: '11:40', end: '13:00' },
  { id: 4, label: '4 пара', start: '13:30', end: '14:50' },
  { id: 5, label: '5 пара', start: '15:00', end: '16:20' },
  { id: 6, label: '6 пара', start: '16:30', end: '17:50' },
  { id: 7, label: '7 пара', start: '18:00', end: '19:20' },
  { id: 8, label: 'Вечер', start: '19:30', end: '20:50' },
];
...
const INITIAL_DATA: ScheduleFormData = {
  discipline: '',
  course: '',
  group: '',
  customPeriod: false,
...
export const ScheduleModal: React.FC<ScheduleModalProps> = ({ isOpen, onClose, onSave }) => {
  ...
  const [formData, setFormData] = useState<ScheduleFormData>(INITIAL_DATA);
  ...
  const handleExit = () => {
    if (JSON.stringify(formData) !== JSON.stringify(INITIAL_DATA)) setShowExitConfirm(true);
      ...
      {formData.days.length > 0 && (
        <div className="space-y-3 animate-in slide-in-from-bottom-2 fade-in duration-300">
           {formData.days.map(day => {
              const timeSlots = Array.isArray(formData.dayTimes[day]) ? formData.dayTimes[day] : [];
              return (
              <div key={day} className="p-4 bg-white rounded-2xl shadow-sm border border-gray-100 flex gap-4 items-start">
                 ...
                 <div className="flex-1 space-y-3">
                   {timeSlots.map((currentTime, idx) => (
                    <div key={idx} className="flex gap-2 items-center">
                        <div className="relative flex-1">
                           <select
                              value={currentTime.start}
                              onChange={(e) => setDayTime(day, idx, 'start', e.target.value)}
                           ...
                           >
                              ...
                              {TIME_SLOTS.map(slot => (<option key={slot.id} value={slot.start}>{slot.start}</option>))}
                           </select>
                        </div>
                        ...
                        <div className="relative flex-1">
                           ...
                           >
                              ...
                              {TIME_SLOTS.map(slot => (<option key={slot.id} value={slot.end}>{slot.end}</option>))}
                           </select>
                        </div>
                        {timeSlots.length > 1 && (
                  ...
                  <div className="space-y-4">
                     {formData.individualJournals.map((journal, idx) => (
                        <div key={journal.id} className="bg-[#F9F9FB] rounded-[20px] p-5 border border-gray-100 space-y-4 relative">
                           ...
                           {journal.days.map(d => {
                              const timeSlots = Array.isArray(journal.dayTimes[d]) ? journal.dayTimes[d] : [];
                              return (
                              <div key={d} className="p-3 bg-white rounded-xl shadow-sm border border-gray-100 flex gap-3 items-start">
                                 ...
                                 <div className="flex-1 space-y-2">
                                    {timeSlots.map((time, idx) => (
                                       <div key={idx} className="flex gap-2 items-center">
                                          <select value={time.start} onChange={(e) => setJournalDayTime(journal.id, d, idx, 'start', e.target.value)} className="w-full bg-[#F2F2F7] rounded-lg px-2 py-1.5 text-[13px] outline-none">
                                             <option value="" disabled>Начало</option>
                                             {TIME_SLOTS.map(slot => (<option key={slot.id} value={slot.start}>{slot.start}</option>))}
                                          </select>
                                          <span className="text-gray-300">-</span>
                                          <select value={time.end} onChange={(e) => setJournalDayTime(journal.id, d, idx, 'end', e.target.value)} className="w-full bg-[#F2F2F7] rounded-lg px-2 py-1.5 text-[13px] outline-none">
                                             <option value="" disabled>Конец</option>
                                             {TIME_SLOTS.map(slot => (<option key={slot.id} value={slot.end}>{slot.end}</option>))}
                                          </select>
                                          {timeSlots.length > 1 && (
