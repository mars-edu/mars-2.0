<template>
  <div>
    <button
      id="copy-intermediate-control-button"
      ...
      aria-label="Копировать промежуточный контроль"
      type="button"
      @click.stop="openPopover"
      :disabled="!hasData"
      :title="hasData ? 'Копировать промежуточный контроль' : 'Нет доступных данных для копирования'"
    >
    ...
    <GuardedPopover
      v-slot="{ requestClose }"
      id="copy-intermediate-control-popover"
      style="width: 600px !important"
      target="#copy-intermediate-control-button"
      :on-closed="resetForm"
    >
      <div class="bg-card text-card-foreground">
        <PopoverHeader
          title="Копировать промежуточный контроль"
          :on-cancel="requestClose"
        />
        <div
          v-if="formError || scheduledIntermediateControlStore.error"
          class="px-4 pt-2 text-destructive text-sm"
        >
          {{ formError || scheduledIntermediateControlStore.error }}
        </div>
        ...
        <div class="p-4 space-y-4">
          <div class="space-y-2">
            ...
            >
              Нет доступных семестров с промежуточным контролем
            </div>
          ...
          >
            <p class="text-sm text-muted-foreground">
              Промежуточный контроль из выбранного семестра будет скопирован в
        ...
        <PopoverFooter
          :on-save="handleCopy"
          :disabled="!selectedSemesterId || scheduledIntermediateControlStore.loading"
          :is-loading="scheduledIntermediateControlStore.loading"
...
const availableSemesters = computed(() => {
  ...
  return allSemesters
    ...
    .map((semester) => {
      const items = scheduledIntermediateControlStore.getScheduledIntermediateControlsBySemester(semester.id);
...
const openPopover = () => {
  ...
  f7.popover.open("#copy-intermediate-control-popover", "#copy-intermediate-control-button");
};

const closePopover = () => {
  f7.popover.close("#copy-intermediate-control-popover");
  resetForm();
};
...
const handleCopy = async () => {
  ...
  try {
    await scheduledIntermediateControlStore.copyFromSemester(
    ...
    f7.toast.show({
      text: "Промежуточный контроль успешно скопирован",
      closeTimeout: 2000,
      position: "center",
    });
    ...
    console.error("Failed to copy intermediate controls:", error);
