<template>
  <div>
    ...
    >
      ...
      <span class="hidden sm:inline">Создать тест</span>
    </button>
    ...
    >
      <div class="cabinet-popover bg-card text-card-foreground flex flex-col h-full max-h-[90vh]">
        <PopoverHeader title="Создание теста" subtitle="Новое тестирование" :on-cancel="requestClose" />
...
const addTestPopupRef = ref<{ allowNextClose: () => void } | null>(null);
...
function saveTest() {
  ...
  emit('save', payload);
  if (addTestPopupRef.value) addTestPopupRef.value.allowNextClose();
  isPopupOpen.value = false;
}
</script>
