<template>
  <div class="mt-6 pt-4 border-t border-border grid grid-cols-2 gap-x-8 gap-y-4">
    ...
    <Input
      ...
      :model-value="step.theoreticalHours"
    ...
    <Input
      ...
      :model-value="step.labPracticalHours"
    ...
    <Input
      ...
      :model-value="step.trainingPracticeHours"
    ...
    <Input
      :id="'individual-hours-'"
      :model-value="step.individualHours"
    ...
    <Input
      ...
      :model-value="step.individualAdditionalHours"
 ...
 *
 * State stays with the parent (many pieces of the form read `step`); this
 ...
 * distributeHoursFromField mutates its `step.distributionEntries` accordingly.
 */
type HourField =
...
export type HourFieldsStep = Partial<Record<HourField, string>> & Record<string, unknown>;
...
const emit = defineEmits<{
  (e: "update:step", value: HourFieldsStep): void;
