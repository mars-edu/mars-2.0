...
> **Прод-статус (2026-07-23):** union-бэкенд + компонент миграций
> ЗАДЕПЛОЕНЫ на прод (festive-cormorant-785); `item.totalHours` мигрирован
> string→number (`migrations/workloads:totalHoursToNumber`, 5 записей).
> `totalHours` пока `v.union` (narrow → `v.number()` после деплоя нового
> фронта). Массив-миграция (semesters) ещё НЕ начата.
...
- **Экспорт: CSV → XLSX через `exceljs` (РЕШЕНО, вариант A).** Заменить
  `downloadWorkload`/`downloadAllWorkloads` на генерацию `.xlsx` (exceljs уже в
  deps + прецедент `convex/excel/lib/workloadExport.ts`). Это:
  - **убивает `src/lib/workloadCsv.ts` + `escapeCsvCell`** (типизированные ячейки
    `cell.value` → инъекция формул невозможна без апострофа-костыля, который
    портил числа; корректный UTF-8 без BOM-хака; числа как числа, без KZ-запятой);
  - **динамика по `item.semesters` (по `semesterDefinition.number`)** вместо
    хардкода 1-2 → чинит «семестр 3+ не экспортируется»;
  - **решить `_ind`** — включать индивидуальную строку отдельной или исключать
    (продуктовый вопрос, см. §8).
  - Тесты `workloadCsv.spec.ts` переписать под XLSX-builder (собрать матрицу
    строк/значений — тестировать её, не DOM/Blob).
...
- **Экспорт хардкод на семестры 1-2 + ручной CSV** (`downloadWorkload`,
  `downloadAllWorkloads`) — семестр 3+ не попадал в выгрузку; `escapeCsvCell`
  анти-инъекция апострофом портила числа. **РЕШЕНО:** переход на XLSX через
  `exceljs` (Фаза 3) — динамика семестров + типизированные ячейки (нет
  инъекции). Поведенческое изменение (формат `.xlsx` вместо `.csv`, новые
  колонки семестров) — упомянуть в PR, согласовать с пользователями.
...
1. ✅ **УСТАНОВЛЕНО (прод-аудит 2026-07-23):** **5 записей** `workloads` (все
   тестовые данные владельца, 1 активный год) → full-scan тривиально, ручная
   сверка легко. См. §9 «Прод-аудит».
2. ~~Формат `totalHours` — оставить `string` или перевести в `number`.~~
   **УСТАРЕЛО (D3, см. §9):** уже `v.union(v.string(), v.number())`, данные
   на проде мигрированы string→number. Открытый остаток — сузить union до
   `v.number()` после деплоя нового фронта.
3. **Экспорт CSV → XLSX (exceljs) — РЕШЕНО (A).** Согласовать с пользователями
   смену формата `.csv`→`.xlsx` + динамические колонки семестров. **И решить
   `_ind`**: включать индивидуальную строку в выгрузку (отдельной строкой) или
   исключать? (сейчас `_ind` попадает в CSV — баг/фича?).
...
## 9. Edge-case пробелы (readiness delta) — ОБЯЗАТЕЛЬНО до Фаз 2+

Из adversarial edge-case аудита плана. **План НЕ готов к исполнению Фаз 2+, пока
не влиты.** Прод-статус — по реальному аудиту данных (см. ниже).

| # | Sev | Сценарий | Фикс |
|---|-----|----------|------|
| **A2** | High | idempotency отравлена пустым массивом: `if (item.semesters)` считает `[]` truthy → item со всеми скипнутыми колонками «готов» навсегда, не ретраится даже после починки данных | `item.semesters?.length > 0` или отдельный маркер |
| **A1** | High | дубли `semesterDefinition.number`: `map.set(number,id)` last-write-wins **молча** (затёртая сторона не логируется) | детект дублей + hard-abort/лог на нагрузку |
| **A3** | High | `workloads.academicYearId` (`v.string()` легаси) не резолвится к `v.id("academicYears")` → `by_academicYear` = `[]` → все колонки скип → пустой массив (+ складывается с A2) | pre-migration аудит висячих `academicYearId` |
| **B1** | High | `addSubjectFromRup` итерит `distributionEntries` БЕЗ year-фильтра (в отличие от `filteredRup`) → после миграции seeding создаёт orphan-`semesterId` межгодовых записей на каждый посев | явный year-фильтр в seeding — пункт Фазы 3 |
| **C1** | High | orphan-политика не задана: `findSemesterEntry` с нерезолвимым `semesterId` → молча выкинут из суммы → `totalHours` дрейфует вниз на ре-сейве | задать политику (бакет «не назначено» / purge с warning / ошибка) + тест |
| **C2** | High | сужение `semesterId` string→`v.id` без cleanup/гейта; `addDistribution` (`:128`, `as any`) продолжает лить невалидные id | pre-narrowing аудит-миграция + починить `addDistribution` до Фазы 5 |
| **D1** | High | `workloads.journalsCreatedSemesters` хранит сырой ординал (startDate-сортировка); смена `mutations.ts:91` на number-матч молча переопределяет «семестр 1/2» → журнальная идемпотентность (`:98-132` wipe+recreate) ломается. Не в инвентаре §1 | ремап/рекей поля в той же миграции |
| **D2** | Med | wizard-табы захардкожены `selectSemester(1)/(2)` (`WorkloadJournalWizard.vue:23,29`), не по `semesterCount` → сем 3+ недостижим для генерации журналов | динамика табов в Фазе 3 |
| **D3** | Low | §8 Q2 (`totalHours` string vs number) устарел — уже `v.union`, мигрирован | ✅ вычеркнуто в §8 |

Минорные: **A4** висячий `semesterDefinitionId` → тихий дроп без лога; **A5** дыры
в нумерации → тихий скип; **E1** full-scan `semesterDefinitions` на каждую
нагрузку (перф); **E2** запятая-локаль в `parseFloat`.

### Прод-аудит (2026-07-23) — реальные данные крошечные + чистые
5 workloads (все данные владельца), 1 активный год «2025-2026» с семестрами
`number` 1&2 в порядке дат, 2 `semesterDefinitions` (1,2), ноль запятых-десятичных,
все `academicYearId` резолвятся. ⇒ **A1 / A3 / E2 / раскол сортировки на текущих
данных НЕ триггерятся.** 9 гэпов = защита кода / будущие годы, а НЕ починка
существующей порчи. Массив-миграция прошла бы на текущих данных чисто, но код-фиксы
(A2/B1/C1/C2/D1/D2) нужны до Фаз 2+ по построению.
