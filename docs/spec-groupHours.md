...
Статус: **✅ РЕАЛИЗОВАНО (2026-07-23), в PR #3, Convex-бэкенд задеплоен на прод.**
Owner-решение (2026-07-22/23): добавить поле «Групповые» как **инпут** на уровне
`rupEntry`; временно **отключить** сверку (красный highlight) полей
теория/практика/срс/срсп («пока не считать — потом уточним»).

**Что сделано:** `groupHours: v.optional(v.string())` в `convex/schema/workloadItem.ts`-соседней
`rup`-схеме + create/update/updateWithDistributions/createMultiLanguage валидаторах
+ `RupEntry` тип + все стор-сайты (transform/create/update/duplicate) + инпут
«Групповые» в `RupEntryPopup` (между Всего/Теоретических) + zod (лениво) + save +
copyFromSource; **все 4 сверки распределения запаркованы** (нейтральный цвет).
Проверено живым смоук-тестом на проде (поле рендерится, валидация не красная).
**Отложено (downstream):** калькулятор #4 (группа vs индив бакеты) + open questions ниже.
